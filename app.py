    ...
    def create_card(self, title, value, icon, color):
        return html.Div(
            ...
            children=[
                ...
                html.H2(f"{value:,}", style={"color": color, "marginTop": "10px", "fontSize": "28px"})
            ]
        )
    ...
    def create_layout(self):
        return html.Div(
            ...
            children=[
                ...
                html.Div([
                    self.create_card("总采集量", self.df["采集数量"].sum(), "database", self.COLOR_SCHEME["highlight"]),
                    self.create_card("已完成项目", len(self.df[self.df["状态"] == "已完成"]), "check-circle", self.COLOR_SCHEME["已完成"][0]),
                    self.create_card("未完成项目", len(self.df[self.df["状态"] == "未完成"]), "times-circle", self.COLOR_SCHEME["未完成"][0])
